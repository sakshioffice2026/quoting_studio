...
def _decimate_path(path, max_pts=20):
    """Down-sample a dense polyline (e.g. a 32-48 segment arc/gothic curve)
    to at most max_pts points before it reaches makePipeShell's mitred
    Frenet-transition solver or the segmented-fuse fallback — both scale
    badly with vertex count and a 50+ point arc path is enough to stall
    past typical request timeouts. Endpoints are always kept; interior
    points are picked at uniform stride so the curve shape is preserved.
    """
    n = len(path)
    if n <= max_pts:
        return path
    stride = (n - 1) / (max_pts - 1)
    raw_idx = [round(i * stride) for i in range(max_pts)]
    idx = sorted(list(dict.fromkeys(raw_idx)))
...
def _make_path_solid_segments(rings, bar, depth, path, closed):
    ...
    # multiFuse() runs one batched BOPAlgo pass over all pieces at once.
    # The previous chained "result = result.fuse(s)" loop re-resolves
    # intersections against an ever-growing accumulated result on every
    # iteration — a known FreeCAD hang/stall pattern for path segment
    # counts like an arched head's ~20-50 pieces. Falls back to the old
    # chained loop only if multiFuse itself fails.
    try:
        result = solids[0].multiFuse(solids[1:])
        if result.isValid():
            return result
    except Exception as e:
        print("multiFuse failed, falling back to chained fuse:", e, flush=True)
    ...
    try:
        if path:
            ...
            if {need_views} or {need_techdraw}:
                try:
                    alt = _make_path_solid_segments(rings, bar, depth, _decimate_path(path), closed)
