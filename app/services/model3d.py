...
def generate_3d_freecad(window, panes, tenant_id=None, fmt='glb') -> bytes:
    ...
    try:
        ...
        if fmt == 'dxf_views':
            ...
            # A DXF header alone (no ENTITIES) means the Draft projection
            # produced no geometry — never hand back a blank drawing.
            txt = data[:20000].decode('latin-1', errors='ignore')
            if 'ENTITIES' not in txt:
                raise RuntimeError(
                    'FreeCAD produced a views DXF with no ENTITIES section '
...
def _build_script(json_path: str, step_path: str,
    ...
    need_views = 'True' if fmt in ('dxf_views',)  else 'False'
...
if not all_solids:
    ...
    # ── 2D multi-view drawing (FreeCAD Draft workbench projection) ─────
    # Reuses the SAME solids already built above for STEP/STL export --
    ...
    if {need_views}:
        ...
        GAP = 100.0  # mm between views on the sheet

        # Non-overlapping layout: front at origin, top above, side to the right
        front_view.Placement.Base = App.Vector(-fx0, -fy0, 0)
        top_view.Placement.Base   = App.Vector(-tx0, -ty0 + fh + GAP, 0)
        side_view.Placement.Base  = App.Vector(-sx0 + fw + GAP, -sy0, 0)
        doc.recompute()

        import importDXF
        importDXF.export([front_view, top_view, side_view], r"{vpath}")
        try:
            _vsz = os.path.getsize(r"{vpath}")
            print(f"2D views DXF exported ({{_vsz}} bytes)", flush=True)
        except Exception as _e:
            print("2D views DXF export check failed:", _e, flush=True)
